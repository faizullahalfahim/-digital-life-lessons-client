  ...
  const links = (
    <div className="flex flex-col gap-1 md:gap-3 text-sm md:text-base font-medium">
      <NavLink
        to="/add-lesson"
        className={({ isActive }) => 
          `flex items-center gap-3 px-3 py-2.5 rounded-lg transition ${isActive ? 'bg-primary/10 text-primary' : 'hover:bg-slate-100'}`
        }
      >
          ...
          <div className="collapse collapse-arrow bg-transparent rounded-lg">
            ...
            <div className="collapse-content bg-slate-50/50 rounded-b-lg">
              <ul className="menu menu-sm p-0 pt-2 gap-1">
                <li><Link to="/dashboard" className="py-2"><LayoutDashboard className="w-4 h-4" /> Overview</Link></li>
                <li><Link to="manage-users" className="py-2"><Users className="w-4 h-4" /> Manage Users</Link></li>
                <li><Link to="manage-lessons" className="py-2"><BookOpen className="w-4 h-4" /> Manage Lessons</Link></li>
                <li><Link to="reported-lessons" className="py-2"><Flag className="w-4 h-4" /> Manage Reported</Link></li>
                <li><Link to="admin-profile" className="py-2"><Settings className="w-4 h-4" /> Admin Profile</Link></li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div className="drawer-content flex flex-col">
        ...
        <div className="navbar bg-white/80 backdrop-blur-md text-gray-800 shadow-sm sticky top-0 z-20 px-2 md:px-4 xl:px-8 h-16 border-b border-gray-100">
          <div className="navbar-start w-auto gap-1">
            <label htmlFor={drawerId} className="btn btn-ghost btn-circle md:flex">
            ...
            {!loading && (
              user ? (
                <div className="dropdown dropdown-end">
                  ...
                  <ul tabIndex={0} className="menu menu-sm dropdown-content bg-white rounded-xl z-[30] mt-3 w-64 p-3 shadow-2xl border border-gray-100">
                    <li className="mb-2 pb-2 border-b">
                      <div className="flex items-center gap-3 hover:bg-transparent">
                        <div className="w-10 h-10 rounded-full overflow-hidden shrink-0">
                          <img src={user.photoURL || "https://i.ibb.co/mR79Y6B/user.png"} alt="avatar" className="w-full h-full object-cover" />
                        </div>
                        <div className="min-w-0">
                          <p className="font-bold truncate text-gray-800">{user.displayName || "User"}</p>
                          <p className="text-xs text-slate-500 truncate">{user.email}</p>
                        </div>
                      </div>
                    </li>
                    <li>
                      <button onClick={handleLogout} className="flex items-center gap-2 text-error py-2.5 rounded-lg hover:bg-red-50">
                ...
                <Link to="/login" className="btn btn-primary btn-sm md:btn-md rounded-full px-5">
            ...
            {user ? (
              <div className="flex flex-col items-center">
                <div className="relative group">
                    ...
                    {role === 'admin' && <div className="absolute bottom-0 right-0 bg-primary text-white text-[10px] px-2 py-0.5 rounded-full shadow-sm">Admin</div>}
                </div>
                ...
                <div className="text-center py-4">
                     <Logo />
                     <p className="text-sm text-gray-500 mt-2 italic">Learn Life Lessons Digitally</p>
                </div>
            )}
          </div>
